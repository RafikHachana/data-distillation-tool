...
def train(model, data, params):
    ...
    for e in range(params["EPOCH"]):
        ...
        for i in range(0, len(data["train_x"]), params["BATCH_SIZE"]):
            ...
            batch_x = Variable(torch.LongTensor(batch_x)).to(params["DEVICE"])
            batch_y = Variable(torch.LongTensor(batch_y)).to(params["DEVICE"])
...
def test(data, model, params, mode="test"):
    ...
    x = Variable(torch.LongTensor(x)).to(params["DEVICE"])
...
def main():
    ...
    if options.mode == "train":
        ...
        model = utils.load_model(params).to(params["DEVICE"])
